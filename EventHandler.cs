...
namespace SCPCanvasPaint
{
    public class EventHandler
    {
        ...
        public void OnDroppingItem(DroppingItemEventArgs ev)
        {
            if (CustomItem.TryGet(ev.Item, out CustomItem customItem) && customItem.Id == Plugin.Singleton.BrushItem.Id)
            {
                ...
                {
                    ev.Player.ShowHint(TranslationManager.Instance.Get("mode_on_hint"), 3f);
                    CheckAndStartRaycast(ev.Player, session);
                }
                else
                {
                    ev.Player.ShowHint(TranslationManager.Instance.Get("mode_off_hint"), 3f);
        ...
        {
            if (!session.DrawCoroutine.IsRunning)
            {
    ...
    : session.CustomPalette[nextIndex - basePaletteCount];
                    ...
                    ev.Player.ShowHint(string.Format(TranslationManager.Instance.Get("color_selected"), "#" + ColorUtility.ToHtmlStringRGB(session.CurrentColor)), 2f);
                }
            }
        }
        ...
        {
            while (session.IsDrawing || session.GrabbedCanvas != null)
            {
                if (session.GrabbedCanvas != null)
                {
                    if (player.CurrentItem == null || !CustomItem.TryGet(player.CurrentItem, out CustomItem drawBrushItem) || drawBrushItem.Id != Plugin.Singleton.BrushItem.Id)
                    {
                        yield return Timing.WaitForSeconds(0.5f);
                        continue;
                    }

                    else if (session.GrabbedCanvas.RootObject != null)
                    {
                        Transform cam = player.CameraTransform;
                        Vector3 targetPos = cam.position + (cam.forward * Mathf.Max(session.GrabOffset.z, 2f));
                        session.GrabbedCanvas.RootObject.transform.position = targetPos;
                        session.GrabbedCanvas.RootObject.transform.rotation = cam.rotation;

                        int width = session.GrabbedCanvas.Grid.GetLength(0);
                        int height = session.GrabbedCanvas.Grid.GetLength(1);
                        float offset = session.GrabbedCanvas.PhysicalSize / session.GrabbedCanvas.Size;
                        float startOffsetX = -session.GrabbedCanvas.PhysicalSize / 2f;
                        float startOffsetY = -(session.GrabbedCanvas.PhysicalSize / session.GrabbedCanvas.Ratio) / 2f;

                        for (int x = 0; x < width; x++)
                        {
                            for (int y = 0; y < height; y++)
                            {
                                Primitive p = session.GrabbedCanvas.Grid[x, y];
                                if (p != null && p.Position.y > -500f)
                                {
                                    Vector3 localPos = new Vector3(startOffsetX + (x * offset) + (offset / 2f), startOffsetY + (y * offset * (1f / session.GrabbedCanvas.Ratio)) + ((offset * (1f / session.GrabbedCanvas.Ratio)) / 2f), 0);
                                    p.Position = session.GrabbedCanvas.RootObject.transform.TransformPoint(localPos);
                                    p.Rotation = session.GrabbedCanvas.RootObject.transform.rotation;
                                }
                            }
                        }
                    }
                ...
                if (Physics.Raycast(cameraTransform.position, cameraTransform.forward, out RaycastHit hit, Plugin.Singleton.Config.MaxDrawDistance, 1 << 0))
                {
                    if (hit.collider.gameObject.TryGetComponent<CanvasPixelData>(out var pixelData))
                    {
                        ...
                        if (targetCanvas != null && targetPrimitive != null)
                        {
                            ...
                            {
                                player.ShowHint(TranslationManager.Instance.Get("private_canvas_error"), 1f);
                            }
                            else
                            {
        ...
        private void ApplyBrushDirect(CanvasInstance canvas, int targetX, int targetY, PlayerPaintSession session, Player player)
        {
            ...
            if (session.IsEraser && !canvas.IsPublic && canvas.OwnerId != player.UserId && !isTrustedEraser && !player.RemoteAdminAccess)
            {
                player.ShowHint(TranslationManager.Instance.Get("private_erase_error"), 1f);
                return;
            }
